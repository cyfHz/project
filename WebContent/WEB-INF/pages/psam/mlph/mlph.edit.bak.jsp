			...
			<fieldset>
				...
				<table class="detail-table">
					<tr>
						<th>地址:</th>
						<td>
							<input type="text" name="DZMC" class="easyui-validatebox" data-options="required:true ,validType:'length[1,200]'"  value="${mlph.DZMC}" />
						</td>
						<th>区划内详细地址:</th>
						<td>
							<input type="text" name="QHNXXDZ" value="${mlph.QHNXXDZ}" class="easyui-validatebox" data-options="required:true ,validType:'length[1,200]'" />
						</td>
					</tr>
					...
					<tr>	
						<th>地址元素类型:</th>
						<td>
							<input id="dzyslxdm" dict="dzyslxDict"  name="DZYSLXDM" value="${mlph.DZYSLXDM}" data-options="editable:false"/>
						</td>
						<th>省市县（区）:</th>
						<td>
							<input name="SSXQMC" value="${mlph.SSXQMC}" id="text_ssxqmc" onclick="openXzqhTreeDialog('setSsxqdm')" class="easyui-validatebox" data-options="required:true" readonly="readonly" />
						</td>
					</tr>
					...
					<tr>
						<th>所属最低一级行政区划:</th>
						<td>
							 <input id="text_sszdyjxzqymc" onclick="openXzqhTreeDialog('setZdxzqhbm')" name="SJXZQYMC"  value="${mlph.SJXZQYMC}" class="easyui-validatebox" data-options="required:true" readonly="readonly" />
						</td>
						<th>警务区名称:</th>
						<td>
							<input onclick="openLookBackjwq();" id="text_jwqmc" type="text" name="JWQMC" value="${mlph.JWQMC}" class="easyui-validatebox" data-options="required:true,validType:'length[1,200]'" readonly="readonly" />
						</td>
					</tr>
					...
					<tr>
						<th>所属街路巷(小区)名称:</th>
						<td>
							<input onclick="openLookBack_jlx()" id="text_ssjlxxq_jlxxqmc" type="text" name="SSJLXXQ_JLXXQMC" value="${mlph.SSJLXXQ_JLXXQMC}" readonly="readonly" />
						</td>
						<th>门(楼)牌号:</th>
						<td>
							<input type="text" name="MLPH" value="${mlph.MLPH}" class="easyui-validatebox" data-options="required:true ,validType:'length[1,150]'"  />
						</td>
					</tr>
					...
					<tr>
						<th>临时门（楼）牌标识:</th>
						<td>
							<input id="lsmlpbs" dict="yesornoDict"  name="LSMLPBS" value="${mlph.LSMLPBS}" data-options="editable:false" />
						</td>
						<th>门楼牌号类型:</th>
						<td>
							<input id="mlphlxid" dict="mlphlxDict"  name="MLPHLXID" value="${mlph.MLPHLXID}" data-options="editable:false"/>
						</td>
					</tr>
					...
					<tr>
						<th>中心点横坐标:</th>
						<td>
							<input type="text" class="easyui-numberbox" id="ZXDHZB" data-options="min:113,max:125,precision:8"  value="${mlph.ZXDHZB}" readonly="readonly" onclick="openEditMark()"/>
						</td>
						<th>中心点纵坐标:</th>
						<td>
							<input type="text" class="easyui-numberbox" id="ZXDZZB" data-options="min:34,max:39,precision:8"  value="${mlph.ZXDZZB}" readonly="readonly" onclick="openEditMark()"/>
						</td>
					</tr>
				</table>
			</fieldset>
...
<div id="dlg_mlph_zb_mark_pgis_edit"  class="easyui-dialog" style="width: 1000px; height: 500px; padding: 0px 0px" onClose="closeGuihuaDialog()" maximizable="true"  modal="true" closed="true" closable="true"  buttons="#dlg_mlph_zb_mark_buttons_edit">
	...
	<div id="dlg_mlph_zb_mark_buttons_edit">
		  ...
		  <a href="#" class="easyui-linkbutton" iconCls="icon-cancel" plain="true" onclick="javascript:$('#dlg_mlph_zb_mark_pgis_edit').dialog('close')">关闭 </a>&nbsp;
	</div>	
</div>	
...
function openLookBack_jlx(){
	...
	var params = {
			sszdyjxzqy_dzbm:sszdyjxzqy_dzbm,
