var content_error_warn_info_correct=
	'<div style="padding:5px;text-align:center ;">'+
		'<div style=" text-align:left ;">'+
			'&nbsp; &nbsp;'+
		    '<a href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:true" icon="icon-#icon#"></a>&nbsp;'+
		    '<font style="font-weight: bold;size: 15em;">#alertTitle#：</font>'+
		    '<hr style="width: 95%;color: gray;" size="1px" noshade="true"/>'+
		'</div>'+
		'<div style="margin-right: auto; margin-left: auto; text-align:left ; width: 95%;height: 76px ;overflow-y:auto;">'+
...
var content_confirm=
	'<div style="padding:5px;text-align:left ;">'+
		'<div>'+
		    ...
		    '<a href="javascript:void(0);" class="easyui-linkbutton"  data-options="plain:true" icon="icon-#icon#"></a>&nbsp;'+
		    '<font style="font-weight: bold;size: 15em;">#alertTitle#：</font>'+
		    '<hr style="width: 95%;color: gray;"  noshade="true" height:1px;border:none;border-top:1px solid #555555;/>'+
		    '</div>'+
		    '<div style=" margin-right: auto; margin-left: auto; width: 95%;height: 66px ;overflow-y:auto;">'+
		    '&nbsp; &nbsp;'+
		    '<font style="size: 13em;" >'+
		    '#alertMessage#'+
		    '</font>'+
...
alertMsg = {
	...
	_keydownOk: function(event){
		if (event.keyCode == KMCORE.keyCode.ENTER) event.data.target.trigger("click");
		return false;
	},
	_keydownEsc: function(event){
		if (event.keyCode == KMCORE.keyCode.ESC) event.data.target.trigger("click");
	},
	...
	_alert: function(type, msg, options) {
		...
		$.extend(op, options);
		var buttons = [
			{name:op.okName, call: op.okCall, keyCode:KMCORE.keyCode.ENTER}
		];
	...
	confirm: function(msg, options) {
		...
		var buttons = [
			{name:op.okName, call: op.okCall, keyCode:KMCORE.keyCode.ENTER},
			{name:op.cancelName, call: op.cancelCall, keyCode:KMCORE.keyCode.ESC}
		];
