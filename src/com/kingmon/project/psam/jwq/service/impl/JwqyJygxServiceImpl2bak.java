...
//@Service
public class JwqyJygxServiceImpl2bak extends BaseService implements IJwqyJygxService{
	...
	public void addJyToJwq(String[] app_userids, String jwqid){
		...
		String sql=" update  ent_jwqyjygx set SFYX='0' where USER_ID in (:app_userids )  and JWQID <>:jwqid ";		
		...
		int a[]=jdbcBaseDao.jdbcBatchUpdate(sql, mapArray);
		
		for(int i=0;i<app_userids.length;i++){
			String selectSql="select USER_ID as userid from ent_jwqyjygx  where USER_ID =:app_userid  and JWQID=:jwqid";
			...
			addSQLParam("app_userid", app_userids[i]).
			addSQLParam("jwqid", jwqid));
			if(listHaveIntable!=null&&listHaveIntable.size()>0){
				String updateSql="update ent_jwqyjygx set  SFYX=:sfyx,XGSJ=:xgsj,QYRQ=:qyrq where USER_ID=:app_userid and JWQID=:jwqid";
				int x=jdbcBaseDao.jdbcUpdate(updateSql, 
						ParamObject.new_NP_NC()
				...
				String sql_insert="INSERT INTO ent_jwqyjygx(ID,JWQID, USER_ID, SFYX, QYRQ, XGSJ,MOVESIGN) VALUES "
												+ "(:id,:jwqid,:user_id,:sfyx,:qyrq,:xgsj,'0')";
				ParamObject po=ParamObject.new_NP_NC();
				  po.addSQLParam("id", UUIDUtil.uuid())
				  .addSQLParam("jwqid", jwqid)
				  .addSQLParam("user_id", app_userids[i])
				  .addSQLParam("sfyx", sfyx)
				  .addSQLParam("qyrq", qyrq) 
				  .addSQLParam("xgsj", xgsj);
				  int x=jdbcBaseDao.jdbcUpdate(sql_insert, po);
			}				
		}
	}	
