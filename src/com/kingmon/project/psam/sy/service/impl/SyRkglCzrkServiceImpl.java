 ...
 */
@Service
public class SyRkglCzrkServiceImpl extends BaseService implements ISyRkglCzrkService {
	@Autowired
	...
	public DataSet loadSyCzrkDataSet(Map<String, String> params) {
		...
		String gmsfhm = params.get("gmsfhm");
		if (gmsfhm != null && !gmsfhm.isEmpty()) {
			params.put("gmsfhm", "%" + gmsfhm + "%");
		}else{
			params.remove("gmsfhm");
		}
		...
		if(dataAuth!=null && !dataAuth.isEmpty()){
			params.put("jwzrqdm",  dataAuth + "%");
		}
		return new DataSet(syRkglCzrkMapper.selectSyCzrkCount(params), syRkglCzrkMapper.selectSyCzrklist(params));
	
	}

	@Override
	public SyRkglCzrk loadSyCzrkDeatil(String rkid) {
		if(rkid==null||"".equals(rkid)){
			throw new ServiceLogicalException("请选择常住人口信息");
		}
		SyRkglCzrk czrk=syRkglCzrkMapper.selectByPrimaryKey(rkid);
		if(czrk==null){
			throw new ServiceLogicalException("常住人员信息为空");
		}
		return czrk;
	}
	 @Transactional(rollbackFor = Exception.class)
	@Override
	public void updateSyCzrkAccInfo(SyRkglCzrk czrk,byte[] imgFile) {
...
//		KAssert.hasText(czrk.getJwzrqdm(), "建筑物责任区代码不能为空");
		...
		SyRkglCzrkPic picInfo=syRkglCzrkPicMapper.selectNotNullPicInfoByZjhm(czrk.getGmsfhm());
		//处理照片
...
//							do nothing
					...
					if(picInfo==null){
						...
						syRkglCzrkPicMapper.updateByPrimaryKeySelective(picInfo);
					}
			
				}
				
		syRkglCzrkMapper.updateByPrimaryKeySelective(czrk);
	}
	@Override
	public DataSet loadSyCzrkDataSet(String jzwfjid) {
		...
		.append(" cz.XJZD, ")
		.append(" cz.CSRQ ");
		sql.append(" @from SY_RKGL_CZRK cz ");
		sql.append(" where 1=1 ");
		if(!SubApStrUtil.isEmptyAfterTrimE(jzwfjid)){
			sql.append(" and cz.JZWFJID=:jzwfjid ");
	...
	public void cancelCzrk(String id) {
		// TODO Auto-generated method stub
		  ...
		  Date nowDate=new Date();
		   ...
		   map.put("zxzt", flag);
		   syRkglCzrkMapper.cancelCzrk(map);
	...
	public void activatesyCzrk(String id) {
		// TODO Auto-generated method stub
		   ...
		   map.put("zxrq",nowDate);
		   map.put("zxzt", flag);
		   syRkglCzrkMapper.activatesyCzrk(map);
	...
	public SyRkglCzrk validateCzrk(String sfzh, String jzwfjid) {
		...
		if(sfzh==null||"".equals(sfzh)){
			throw new ServiceLogicalException("身份证号不能为空");
		}
		if(jzwfjid==null||"".equals(jzwfjid)){
			throw new ServiceLogicalException("房间号不能为空");
		}
		return syRkglCzrkMapper.selectCzrkbysfzhandjzwfj(sfzh,jzwfjid);
	}
	...
	public List<SyRkglCzrk> selectSyCzrkInfoByJzwfjId(String jzwfjid) {
		...
		List<SyRkglCzrk> czrklist=syRkglCzrkMapper.selectSyCzrkInfoByJzwfjId(jzwfjid);
		return czrklist;
	}
	...
	public List<SyRkglCzrk> selectSyCzrkInfoByJzwfjIdKey(String jzwfjid){
		...
		List<SyRkglCzrk> czrklist=syRkglCzrkMapper.selectSyCzrkInfoByJzwfjIdKey(jzwfjid);
		return czrklist;
	}
	...
	public List<SyRkglCzrk> selectSyCzrkInfoFwByJzwfjIdKey(String jzwfjid){
		...
		List<SyRkglCzrk> czrklist=syRkglCzrkMapper.selectSyCzrkInfoFwByJzwfjIdKey(jzwfjid);
		return czrklist;
	}
	...
	public SyRkglCzrk selectCzrkInfoByZjbh(String sfzh) {
		...
		return syRkglCzrkMapper.selectRkglCzrkById(sfzh);
	}
...
//		strReturns.put("CSRQ", "1990-04-04");
...
//				String day=csrq.substring(8, 10);
				String year=csrq.substring(0, 4);
			...
			if(strReturns.get("XP")!=null && !("".equals(strReturns.get("XP")))){
				SyRkglCzrkPic picInfo=syRkglCzrkPicMapper.selectPicInfoByZjhm(sfzh);
    ...
    public DataSet selectSyCzrkForFw(Map<String, String> map){
		...
		DataSet ds = new DataSet(syRkglCzrkMapper.selectSyCzrkForFwCount(map), syRkglCzrkMapper.selectSyCzrkForFw(map));
		return ds;
    }
	
	//2016-0816 房屋内 历史常住人口
	@Transactional(rollbackFor = Exception.class,readOnly=true)
	public DataSet selectSyCzrkForFwLsry(Map<String, String> map){
		...
		PaginationUtil.initPageAndSort(map);
		DataSet ds = new DataSet(syRkglCzrkMapper.selectSyCzrkForFwLsryCount(map), syRkglCzrkMapper.selectSyCzrkForFwLsry(map));
		return ds;
	}
}
