...
@Service
public class JzwjgServiceImpl extends BaseService implements IJzwjgService{
	...
	*/
	...
	//----------------2016-0808-------------连续单元号--------------//
		String sflx=(String)(view.get("sflx"));
...
//			Jzwdy jzwdy = JzwJgUtil.constructJzwDy(jzwjg.getJzwjgid(), JzwJgUtil.processDyLcXh(("" + (d + 1)), 3), dymc, (short)(lcs+dxcsx), fjs);
			...
			for (int c = 0; c < lcs; c++) {
				...
				for (int f = 0; f < fjs; f++) {
					...
					// 处理房间名称 长度+后缀
//					String fjmc = (c + 1) + ((f + 1) < 10 ? ("0" + (f + 1)) : ("" + (f + 1)));//2016-0808 使用 是否连续替换
					
					String fjmc="";//房间序号 是否连续---16-0808---
					if(sflx!=null&&sflx.equals("02")){
						fjmc = (c + 1) + ((d*fjs+(f + 1)) < 10 ? ("0" + (d*fjs+(f + 1))) : ("" + (d*fjs+(f + 1))));
					}else{
						fjmc = (c + 1) + ((f + 1) < 10 ? ("0" + (f + 1)) : ("" + (f + 1)));
					}
					...
					// 不再带有单元楼层前缀
//					String fjxh = (c + 1) + ((f + 1) < 10 ? ("0" + (f + 1)) : ("" + (f + 1)));//2016-0808 使用 是否连续替换
					
					String fjxh="";//房间序号 是否连续---16-0808---
					if(sflx!=null&&sflx.equals("02")){
						fjxh = (c + 1) + ((d*fjs+(f + 1)) < 10 ? ("0" + (d*fjs+(f + 1))) : ("" + (d*fjs+(f + 1))));
					}else{
						fjxh = (c + 1) + ((f + 1) < 10 ? ("0" + (f + 1)) : ("" + (f + 1)));
					}
					System.out.println("fjxh_lx:"+fjxh);
...
//						String fjmc = "-" + (c + 1) + ((f + 1) < 10 ? ("0" + (f + 1)) : ("" + (f + 1)));//2016-0808 使用 是否连续替换
						...
						// String fjxh=(d+1)+"-(-"+(c+1)+")-"+(c+1)+"0"+(f+1);//2016-0808 使用 是否连续替换
...
//----------------------validate--------------------------------------------------------------------------------------------------
	...
	public void updateJzwjg(Map<?, ?> view){
		...
		if(dyList!=null&&dyList.size()>0){
			if(dyList.size()>Short.parseShort(jg_dys)){
				AlertSLEUtil.Error("该建筑物实际单元个数：【"+dyList.size()+"】大于【"+jg_dys+"】,请重新填写");
			}
		}
		//jzwlcMapper.selectDistinctSortedLcxhByJzwJgid(jzwjgId)
		...
		for(Jzwdy dy:dyList){
			//单元 每层楼数如果为空或者等于0 则更新当前的 每层门数，反之则不更新
			if(dy.getDyms()==null || dy.getDyms()==0){
				dy.setDyms(Short.parseShort(jg_mdyms));
				jzwdyMapper.updateDyMcForDyId(dy);
			}
		}
		jzwjgMapper.updateByPrimaryKeySelective(jzwjg);
...
//----------------------------------------------------------------------------------------------------------------------------------
		...
		if("dy".equals(jgtype)){
			...
			if(fgList!=null&&fgList.size()>0){
				for(Jzwfj fgx:fgList){
					try{//保存时，判断房间序号是否存在，接收序号时，原数据可能包含有汉字
						Short.parseShort(fgx.getFjxh());
					}catch(NumberFormatException e){
						continue;
					}
	...
	public void addJzwjgDy_20160521(Map<?, ?> view) {
		...
		jzwjg.setDys((short)((short)(jzwjg.getDys()+1)));
		...
		short max_dy_xhx=Short.MIN_VALUE;
		List<Jzwdy> dyList=jzwdyMapper.selectSortedJzwdyByJzwJgid(jzwjgid);
		if(dyList!=null&&dyList.size()>0){
			for(Jzwdy dyx:dyList){
				if(max_dy_xhx<Short.parseShort(dyx.getDyxh())){
					max_dy_xhx=Short.parseShort(dyx.getDyxh());
				}
				try{
		...
		if(max_dy_xhx>0&&max_dy_xhx+1<Short.parseShort(dy_xh)){
			AlertSLEUtil.Error("您输入的单元序号：【"+dy_xh+"】,与最大单元序号：【"+max_dy_xhx+"】，存在不连续情况，请重新输入。");
		}
		...
		for(int c=0;c<dxcs;c++){
			Jzwlc lc=JzwJgUtil.constructJzwLc(jzwjgid, jzwjgdyid, "-" + (c+1), (c+1)+"层");
			lcList.add(lc);
			//生成房间：
			
			for(int f=0;f<dxmcms;f++){
				int width = KConstants.jzwWidth;// 120;
				int height = KConstants.jzwHeight;// 70;
				int left = ((dxmcms * d) + f) * width;
				//int top = (lcs - (c + 1)) * height;
				int ls_totalH = lcs * KConstants.jzwHeight;
				int dx_top = c * height + ls_totalH;
				//String fjxh = (c+ 1) + ((f + 1) < 10 ? ("0" + (f + 1)) : ("" + (f + 1)));
				String fjxh = "-" + (c + 1) + ((f + 1) < 10 ? ("0" + (f + 1)) : ("" + (f + 1)));
				String fjmc=fjxh+"室";
				JzwFjShowInPage fjInPage=new JzwFjShowInPage(1, left, dx_top, width, height);
				Jzwfj fj=JzwJgUtil.constructJzwFj(jzwjgid, jzwdy.getJzwdyid(), lc.getJzwlcid(), fjxh, fjmc, fjInPage);
				fjList.add(fj);
			}
		...
		Jzwjg jgNew=new Jzwjg();
		Long dyCount=jzwdyMapper.selectJzwDyCountByJzwjgId(jzwjgid);
		dyCount=dyCount==null?0:dyCount;
		jgNew.setJzwjgid(jzwjg.getJzwjgid());
//		jgNew.setDys((short)(jzwjg.getDys()+1));
		jgNew.setDys(dyCount.shortValue());
		jzwjgMapper.updateByPrimaryKeySelective(jgNew);
	}
	@Autowired
	IJzwfjService IJzwfjService;
	
	@Override
	@Transactional(rollbackFor=Exception.class)
	public void delJzwjgDy_20160521(String jzwjgid,String dyid) {
		KAssert.hasText(dyid, "请选择要删除的单元");
		Jzwdy jzwdy=jzwdyMapper.selectByPrimaryKey(dyid);
		if(jzwdy==null){
			AlertSLEUtil.Error("未查询到单元数据");
		}
		List<Jzwdy> jzwdyList=jzwdyMapper.selectSortedJzwdyByJzwJgid(jzwjgid);
		if(jzwdyList!=null&&jzwdyList.size()>0){
			for(Jzwdy dyx:jzwdyList){
				try{
					short dy_xh_from_param=Short.parseShort(jzwdy.getDyxh());
					short dyx_from_db=Short.parseShort(dyx.getDyxh());
					if(dy_xh_from_param<dyx_from_db){
						AlertSLEUtil.Error("只能删除最大序号单元");
					}
				}catch(NumberFormatException e){
					if(dyx.getDyxh()==null||dyx.getDyxh().isEmpty()){
						AlertSLEUtil.Error("该建筑物结构中，存在单元序号为空的数据，不符合规范，<br/>请首先修改后进行本操作。");
		...
		List<Jzwfj> fjList=jzwfjMapper.selectJzwfjByJzwDyId(dyid);
		if(fjList!=null&&fjList.size()>0){
			for(Jzwfj fjx:fjList){
				KJSONMSG msg=IJzwfjService.iSjzwfjHaveDpendInfo(fjx.getJzwfjid());
				if(msg.getStatusCode()!=200){
					AlertSLEUtil.Error("该单元内：房间序号：【"+fjx.getFjxh()+"】："+msg.getMessage());
				}
			}
		}
		String del_fj_sql="delete from DZ_JZWFJ where JZWDYID=:jzwdyid";
		jdbcBaseDao.jdbcDelete(del_fj_sql, ParamObject.new_NP_NC().addSQLParam("jzwdyid", dyid));
		...
		Jzwjg jg=jzwjgMapper.selectByPrimaryKey(jzwjgid);
		if(jg==null){
			AlertSLEUtil.Error("未查询到结构数据");
		}
		Jzwjg jgNew=new Jzwjg();
		jgNew.setJzwjgid(jg.getJzwjgid());
		jgNew.setDys((short)(jg.getDys()-1));
		jzwjgMapper.updateByPrimaryKeySelective(jgNew);
...
//	可以添加地下楼层？？？？？
	...
	public void addJzwjgLc(Map<?, ?> view) {
		...
		Jzwjg jg=jzwjgMapper.selectByPrimaryKey(jzwjgid);
		if(jg==null){AlertSLEUtil.Error("未找到该结构信息");}
...
//		}
			...
			for(Jzwlc lcx:lcList){
				try{
					...
					if(lcxh_from_param==lcxh_from_db){
						AlertSLEUtil.Error("该楼层 序号：【"+lcxh_from_db+"】已经存在");
					}
					if((lcxh_from_param>0)&&(lcxh_from_param<lcxh_from_db)){
						AlertSLEUtil.Error("只能添加最大序号楼层");
					}
					if((lcxh_from_param<0)&&(lcxh_from_param>lcxh_from_db)){
						AlertSLEUtil.Error("只能添加地下最小序号楼层");
					}
				}catch(NumberFormatException e){
		...
		short p_lcxh=(short)(lcxh_from_param<0?(lcxh_from_param==-1?1:lcxh_from_param+1):lcxh_from_param-1);
		Jzwlc preorierLc=JzwJgNewUtil.genLcByxh(p_lcxh, lcList);
		if(preorierLc==null){
			AlertSLEUtil.Error("相接处楼层 不存在，请检查数据");
		}
		List<Jzwfj> fjList=jzwfjMapper.selectSortedJzwfjByJzwlcId(preorierLc.getJzwlcid());
		if(fjList==null||fjList.size()==0){
			AlertSLEUtil.Error("相接处楼层不存在房间数据，请联系管理员");
		}
		Jzwfj firstFj=fjList.get(0);
		
		Jzwlc lc=JzwJgUtil.constructJzwLc(jzwjgid,jzwjgDyid, lc_xh, lc_mc);
		jzwlcMapper.insertSelective(lc);
		if(lcxh_from_param>0){//地上。。。
			short dyms=dy.getDyms();
			for(int f=0;f<dyms;f++){
				int width = KConstants.jzwWidth;// 120;
				int height = KConstants.jzwHeight;// 70;
				int left=JzwJgUtil.getLeft(firstFj)+f*width;
				int top = JzwJgUtil.getTop(firstFj)-height;
				String p_LcXh=preorierLc.getLcxh();
				short p_LcXh_short=Short.parseShort(p_LcXh);
				String fjxh = (p_LcXh_short+1 ) + ((f + 1) < 10 ? ("0" + (f + 1)) : ("" + (f + 1)));
				String fjmc=fjxh+"室";
				JzwFjShowInPage fjInPage=new JzwFjShowInPage(1, left, top, width, height);
				Jzwfj fj=JzwJgUtil.constructJzwFj(jzwjgid, dy.getJzwdyid(), lc.getJzwlcid(), fjxh, fjmc, fjInPage);
				jzwfjMapper.insertSelective(fj);
			}
			long lcmaxcount=jzwlcMapper.selectMaxJzwlcDSCountGroupByJzwDyid(jzwjgid);
			if(lcmaxcount>jg.getLcs()){//坐标向下移动
				Jzwjg jgx=new Jzwjg();
				jgx.setJzwjgid(jzwjgid);
				jgx.setLcs(new Long(lcmaxcount).shortValue());
				jzwjgMapper.updateByPrimaryKeySelective(jgx);
				translationPostion(jzwfjMapper.selectFjByJzwjgId(jzwjgid));
			}
			dy.setLcs((short)(dy.getLcs()+1));
			jzwdyMapper.updateByPrimaryKeySelective(dy);
		}else{//l地下楼层
			short dyms=jg.getDxmcms();
			if(dyms<=0){
				AlertSLEUtil.Error("该建筑物地下每层门数未指定");
			}
			for(int f=0;f<dyms;f++){
				int width = KConstants.jzwWidth;// 120;
				int height = KConstants.jzwHeight;// 70;
				...
				int left=JzwJgUtil.getLeft(firstFj)+f*width;
				int top = JzwJgUtil.getTop(firstFj);
				top+=height;
			
				String fjxh =((lcxh_from_param ) + ((f + 1) < 10 ? ("0" + (f + 1)) : ("" + (f + 1))));
				String fjmc=fjxh+"室";
				JzwFjShowInPage fjInPage=new JzwFjShowInPage(1, left, top, width, height);
				Jzwfj fj=JzwJgUtil.constructJzwFj(jzwjgid, dy.getJzwdyid(), lc.getJzwlcid(), fjxh, fjmc, fjInPage);
				jzwfjMapper.insertSelective(fj);
			}
			Long maxdxcs=jzwlcMapper.selectMaxJzwlcDXCountGroupByJzwDyid(jzwjgid);
			maxdxcs=(maxdxcs==null?0:maxdxcs);
			if(jg.getDxcs()<maxdxcs){
				Jzwjg jgx=new Jzwjg();
				jgx.setJzwjgid(jzwjgid);
				jgx.setDxcs((short)(jg.getDxcs()+1));
				jzwjgMapper.updateByPrimaryKeySelective(jgx);
			}
		}
...
//		jzwjgMapper.updateByPrimaryKeySelective(jzwjg);
	...
	public void delJzwjgLc(String jzwjgid,String lcid) {
		...
		Jzwlc lc=jzwlcMapper.selectByPrimaryKey(lcid);
		if(lc==null){
			AlertSLEUtil.Error("未查询到该楼层信息");
		}
		List<Jzwfj> fjList=jzwfjMapper.selectJzwfjByJzwLcId(lcid);
		if(fjList!=null&&fjList.size()>0){
			for(Jzwfj fjx:fjList){
				KJSONMSG msg=IJzwfjService.iSjzwfjHaveDpendInfo(fjx.getJzwfjid());
				if(msg.getStatusCode()!=200){
		...
		short lc_xh_from_paramx=0;
		try{
			lc_xh_from_paramx=Short.parseShort(lc.getLcxh());
		}catch(NumberFormatException e){
			AlertSLEUtil.Error("该建筑物结构中，单元序号：【"+lc_xh_from_paramx+"】,不是数字，不符合规范，<br/>请首先修改后进行本操作。");
		}
		if(lc_xh_from_paramx<0){//地下楼层
			List<Jzwlc> jzwlcList=jzwlcMapper.selectSortedJzwlcByJzwdyid(lc.getJzwdyid());
			if(jzwlcList!=null&&jzwlcList.size()>0){
				for(Jzwlc lcx:jzwlcList){
					try{
						short lc_xh_from_param=Short.parseShort(lc.getLcxh());
						short lcx_from_db=Short.parseShort(lcx.getLcxh());
						if(lcx_from_db>0){
							continue;
						}
						if(lc_xh_from_param>lcx_from_db){
							AlertSLEUtil.Error("只能删除最大序号楼层");
						}
					}catch(NumberFormatException e){
						if(lcx.getLcxh()==null||lcx.getLcxh().isEmpty()){
							AlertSLEUtil.Error("该建筑物结构中，存在单元序号为空的数据，不符合规范，<br/>请首先修改后进行本操作。");
						}
						AlertSLEUtil.Error("该建筑物结构中，单元序号：【"+lcx.getLcxh()+"】,不是数字，不符合规范，<br/>请首先修改后进行本操作。");
					}
				}
			}
			Long maxDxLcsA=jzwlcMapper.selectMaxJzwlcDXCountGroupByJzwDyid(jzwjgid);
			maxDxLcsA=maxDxLcsA==null?0:maxDxLcsA;
			jzwfjMapper.deletejzwfjByLcid(lcid);
			jzwlcMapper.deleteByPrimaryKey(lcid);
			
			Long maxDxLcsB=jzwlcMapper.selectMaxJzwlcDXCountGroupByJzwDyid(jzwjgid);
			maxDxLcsB=maxDxLcsB==null?0:maxDxLcsB;
					
			Jzwjg jg=new Jzwjg();
			jg.setDxcs(maxDxLcsB.shortValue());
			jg.setJzwjgid(jzwjgid);
			jzwjgMapper.updateByPrimaryKeySelective(jg);
			for(int i=0;i<(maxDxLcsA-maxDxLcsB);i++){
				//translationPostion(jzwfjMapper.selectFjByJzwjgId(jzwjgid));
			}
			...
			Long maxLcsA=jzwlcMapper.selectMaxJzwlcDSCountGroupByJzwDyid(jzwjgid);
			maxLcsA=maxLcsA==null?0:maxLcsA;
			jzwfjMapper.deletejzwfjByLcid(lcid);
			jzwlcMapper.deleteByPrimaryKey(lcid);
			Jzwdy jzwdy=jzwdyMapper.selectByPrimaryKey(lc.getJzwdyid());
			Jzwdy dy=new Jzwdy();
			dy.setJzwdyid(jzwdy.getJzwdyid());
			dy.setLcs((short)(jzwdy.getLcs()-1));
			jzwdyMapper.updateByPrimaryKeySelective(dy);
			
			Long maxLcsB=jzwlcMapper.selectMaxJzwlcDSCountGroupByJzwDyid(jzwjgid);
			maxLcsB=maxLcsB==null?0:maxLcsB;
			Jzwjg jg=new Jzwjg();
			jg.setLcs(maxLcsB.shortValue());
			jg.setJzwjgid(jzwjgid);
			jzwjgMapper.updateByPrimaryKeySelective(jg);
			for(int i=0;i<(maxLcsA-maxLcsB);i++){
				translationPostion_top(jzwfjMapper.selectFjByJzwjgId(jzwjgid));
			}
		}

		
	}
	private void translationPostion_top(List<Jzwfj> fjList){
		for(Jzwfj fj :fjList){
			int topx=JzwJgUtil.getTop(fj);
			String shwoInfo=fj.getShowInfo();
			String[] sds=shwoInfo.split(",");
	...
	public void delJzwjgfj(String jzwjgid,String fjid) {
		KAssert.hasText(fjid, "请选择要删除的 数据");
		Jzwfj fj=jzwfjMapper.selectByPrimaryKey(fjid);
		if(fj!=null){
			KJSONMSG msg=IJzwfjService.iSjzwfjHaveDpendInfo(fj.getJzwfjid());
			if(msg.getStatusCode()!=200){
				AlertSLEUtil.Error("该单元内：房间序号：【"+fj.getFjxh()+"】："+msg.getMessage());
			}
		}
		String del_fj_sql="delete from DZ_JZWFJ where JZWFJID=:jzwfjid";
		jdbcBaseDao.jdbcDelete(del_fj_sql, ParamObject.new_NP_NC().addSQLParam("jzwfjid", fjid));
	}
//------------------------------------------------------------------------------------------------------------
//----------------以下函数禁用------------------------------------------------------------------------------------------\
